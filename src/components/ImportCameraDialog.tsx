...
import { X, Upload, FileSpreadsheet, FileUp, AlertTriangle, CheckCircle, Cctv } from 'lucide-react';
import { useTranslation } from 'react-i18next';
import styles from './ImportCameraDialog.module.css';
import { importCameras } from '../services/cameraService';

interface ImportError { row: number; field: string; message: string; }
interface ImportedCamera { id: string; name: string; ip: string; }
...
interface ImportCameraDialogProps {
    onClose: () => void;
    onImported: () => void;
}
...
const ImportCameraDialog: React.FC<ImportCameraDialogProps> = ({ onClose, onImported }) => {
        ...
        <div className={styles.backdrop}>
            <div className={styles.dialog}>
                ...
                <div className={styles.header}>
                    ...
                    <div className={styles.title}>{t('importCamera.title')}</div>
                                    ...
                                    <div className={styles.tagList}>
                                        {result.imported.map(cam => (
                                            <span key={cam.id} className={styles.cameraTag}>
                                                <Cctv size={12} />
                                                {cam.name}
                                                <span className={styles.camIp}>({cam.ip})</span>
                                            </span>
                                        ))}
                                    </div>
                                </div>
                ...
                <div className={styles.footer}>
                    ...
                    >
                        {loading ? <span className={styles.spinner} /> : <Upload size={15} />}
...
export default ImportCameraDialog;
